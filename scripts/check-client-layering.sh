...
#   - client/core is a leaf: it must not import any flux/client package.
...
# core must not import any flux/client package.
if grep -rn --include='*.go' '"github.com/GrayCodeAI/flux/client' client/core/ | grep -v '/client/core"'; then
  echo "FAIL: client/core must not import other flux/client packages" >&2
  fail=1
fi
...
for dir in client/*/; do
  name=$(basename "$dir")
  [ "$name" = "core" ] && continue
  if grep -rn --include='*.go' '"github.com/GrayCodeAI/flux/client' "$dir" | grep -v "/client/core\"" ; then
