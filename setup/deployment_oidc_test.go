...
func TestProviderForDeploymentBedrockOIDCSkippedWhenDisabled(t *testing.T) {
	// In Actions but neither FLUX_OIDC nor roleARN/audience set: OIDC must be skipped.
	t.Setenv("GITHUB_ACTIONS", "true")
	t.Setenv("FLUX_OIDC", "")
...
func TestProviderForDeploymentBedrockOIDCSkippedOutsideActions(t *testing.T) {
	// FLUX_OIDC=1 but not in GitHub Actions: OIDC must be skipped, fall back to stored.
