...
func TestMockProviderErrorMode(t *testing.T) {
	...
	if err.Error() != "flux: mock error" {
		t.Errorf("error = %q, want %q", err.Error(), "flux: mock error")
	}
}
...
func TestMockProviderToolUseMode(t *testing.T) {
	...
	msgs := []FluxMessage{{Role: "user", Content: "search for hello"}}
	resp, err := mp.Chat(context.Background(), msgs, ChatOptions{})
	if err != nil {
		t.Fatalf("Chat: %v", err)
	}
...
func TestMockProviderCallRecording(t *testing.T) {
	...
	msgs := []FluxMessage{{Role: "user", Content: "first"}}
...
func TestMockProviderMultipleCalls(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		mp.Chat(context.Background(), []FluxMessage{{Role: "user", Content: "msg"}}, ChatOptions{})
	}
	if mp.CallCount() != 5 {
...
func TestMockProviderReset(t *testing.T) {
	t.Parallel()
	mp := NewMockProvider(MockModeEcho)
	mp.Chat(context.Background(), []FluxMessage{{Role: "user", Content: "test"}}, ChatOptions{})
	if mp.CallCount() != 1 {
...
func TestMockProviderContextCancellation(t *testing.T) {
	...
	_, err := mp.Chat(ctx, []FluxMessage{{Role: "user", Content: "test"}}, ChatOptions{})
	if err == nil {
...
func TestMockProviderStreamChatToolUse(t *testing.T) {
	...
	msgs := []FluxMessage{{Role: "user", Content: "compute"}}
