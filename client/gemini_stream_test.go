...
func drainGeminiStream(t *testing.T, sr *StreamResult, timeout time.Duration) []FluxStreamEvent {
	t.Helper()
	var out []FluxStreamEvent
...
func TestGemini_Stream_SharedParser_Text(t *testing.T) {
	...
	sr, err := c.StreamChat(context.Background(), []FluxMessage{
...
func TestGemini_Stream_SharedParser_ToolCall(t *testing.T) {
	...
	var toolEvts []FluxStreamEvent
	var doneEvt *FluxStreamEvent
	for i := range events {
...
func TestGemini_Stream_SharedParser_ContextCancel(t *testing.T) {
	...
	sr, err := c.StreamChat(ctx, []FluxMessage{
		{Role: "user", Content: "x"},
	}, ChatOptions{Model: "gemini-test"})
	if err != nil {
		t.Fatalf("StreamChat: %v", err)
...
func TestGemini_Stream_SharedParser_LargeEvent(t *testing.T) {
	...
	var contentEvt *FluxStreamEvent
	for i := range events {
...
// FLUX_GEMINI_SHARED_PARSER=0, the old streamLoop path is used.
...
// collectFluxStreamEvents reads events until the channel closes
// or the deadline fires. Used by the processGeminiStream direct tests.
func collectFluxStreamEvents(t *testing.T, ch <-chan FluxStreamEvent, timeout time.Duration) []FluxStreamEvent {
	t.Helper()
	deadline := time.NewTimer(timeout)
	defer deadline.Stop()
	var out []FluxStreamEvent
