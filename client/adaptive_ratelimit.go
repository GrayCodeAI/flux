...
func NewAdaptiveRateLimitProvider(inner Provider, config AdaptiveRateLimitConfig) (*AdaptiveRateLimitProvider, error) {
	if inner == nil {
		return nil, errors.New("flux: NewAdaptiveRateLimitProvider inner provider must not be nil")
	}
...
func (a *AdaptiveRateLimitProvider) Chat(ctx context.Context, messages []FluxMessage, opts ChatOptions) (*FluxResponse, error) {
...
func (a *AdaptiveRateLimitProvider) StreamChat(ctx context.Context, messages []FluxMessage, opts ChatOptions) (*StreamResult, error) {
	if err := a.checkAndWait(ctx); err != nil {
		return nil, err
	}
...
func (a *AdaptiveRateLimitProvider) checkAndWait(ctx context.Context) error {
	...
	if a.rpmLimit > 0 {
		...
		if remaining <= 0 || a.isNearLimit(remaining, a.rpmLimit) {
			...
			if remaining <= 0 && delay > a.config.MaxDelay {
				a.mu.Unlock()
				return fmt.Errorf("flux: adaptive ratelimit: %s RPM limit reached (%d/%d), resets in %v",
	...
	if a.tpmLimit > 0 {
		...
		if remaining <= 0 || a.isNearLimit(remaining, a.tpmLimit) {
			...
			if remaining <= 0 && delay > a.config.MaxDelay {
				a.mu.Unlock()
				return fmt.Errorf("flux: adaptive ratelimit: %s TPM limit reached (%d/%d), resets in %v",
...
func (a *AdaptiveRateLimitProvider) extractTokens(usage *FluxUsage) int {
	if usage == nil {
		return 0
	}
