...
func TestAnthropicChat_Error401(t *testing.T) {
	...
	_, err := client.Chat(context.Background(), []FluxMessage{
...
func TestAnthropicChat_Error429_Retry(t *testing.T) {
	...
	resp, err := client.Chat(context.Background(), []FluxMessage{
		{Role: "user", Content: "hi"},
	}, ChatOptions{Model: "claude-sonnet-4-6"})
	if err != nil {
...
func TestAnthropicStreamChat_ErrorResponse(t *testing.T) {
	...
	client := NewAnthropicClient("key", server.URL, WithRetry(NewRetryConfig(0, 0, 0)))
	_, err := client.StreamChat(context.Background(), []FluxMessage{
		{Role: "user", Content: "hi"},
	}, ChatOptions{Model: "claude-sonnet-4-6"})
	if err == nil {
...
func TestAnthropicChat_FullToolRoundTrip(t *testing.T) {
	...
	// First call
	resp1, err := client.Chat(context.Background(), []FluxMessage{
		{Role: "user", Content: "What time is it?"},
	}, ChatOptions{
		Model: "claude-sonnet-4-6",
		Tools: []FluxTool{{Name: "get_time", Description: "Get current time", Parameters: map[string]interface{}{"type": "object"}}},
	})
	if err != nil {
	...
	resp2, err := client.Chat(context.Background(), []FluxMessage{
		{Role: "user", Content: "What time is it?"},
...
func TestAnthropicResponseFormatJSONSchema(t *testing.T) {
	...
	_, body, err := c.BuildAnthropicRequest(context.Background(), []FluxMessage{{Role: "user", Content: "hi"}}, ChatOptions{
		Model: "claude-test",
...
func TestAnthropicChat_ThinkingBlocksInResponse(t *testing.T) {
	...
	resp, err := client.Chat(context.Background(), []FluxMessage{{Role: "user", Content: "What is the answer?"}}, ChatOptions{
		Model:                "claude-sonnet-4-6",
...
func TestAnthropicChat_RedactedThinkingSkipped(t *testing.T) {
	...
	client := NewAnthropicClient("sk-test", server.URL, WithRetry(NewRetryConfig(0, 0, 0)))
	resp, err := client.Chat(context.Background(), []FluxMessage{{Role: "user", Content: "Hi"}}, ChatOptions{Model: "claude-sonnet-4-6"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestAnthropicRequest_WithToolChoice(t *testing.T) {
	...
	client := NewAnthropicClient("sk-test", server.URL, WithRetry(NewRetryConfig(0, 0, 0)))
	_, err := client.Chat(context.Background(), []FluxMessage{{Role: "user", Content: "search"}}, ChatOptions{
		Model:      "claude-sonnet-4-6",
		ToolChoice: &ToolChoiceOption{Type: "tool", Name: "search"},
		Tools:      []FluxTool{{Name: "search", Description: "Search", Parameters: map[string]interface{}{"type": "object"}}},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestAnthropicRequest_WithTopPAndStopSequences(t *testing.T) {
	...
	client := NewAnthropicClient("sk-test", server.URL, WithRetry(NewRetryConfig(0, 0, 0)))
	_, err := client.Chat(context.Background(), []FluxMessage{{Role: "user", Content: "Go"}}, ChatOptions{
		Model:         "claude-sonnet-4-6",
		TopP:          float64Ptr(0.8),
