...
func ChatWithContinuation(ctx context.Context, p Provider, messages []FluxMessage, opts ChatOptions, cfg ContinuationConfig) (*FluxResponse, error) {
	if cfg.MaxContinuations <= 0 {
		cfg.MaxContinuations = 3
	}

	var accumulated strings.Builder
	var finalUsage *FluxUsage
	var finalToolCalls []ToolCall
	msgs := make([]FluxMessage, len(messages))
	copy(msgs, messages)
	...
	for i := 0; i <= cfg.MaxContinuations; i++ {
		...
		if i < cfg.MaxContinuations {
			msgs = append(msgs,
				FluxMessage{Role: "assistant", Content: accumulated.String()},
				FluxMessage{Role: "user", Content: "Continue."})
		}
	}
...
func StreamChatWithContinuation(ctx context.Context, p Provider, messages []FluxMessage, opts ChatOptions, cfg ContinuationConfig) (*StreamResult, error) {
	if cfg.MaxContinuations <= 0 {
		cfg.MaxContinuations = 3
	}
	...
	outCh := make(chan FluxStreamEvent, streamChannelBuffer)
	...
	go func() {
		...
		for attempt := 0; attempt <= cfg.MaxContinuations; attempt++ {
			...
			if err != nil {
				emit(cancelCtx, outCh, FluxStreamEvent{Type: "error", Error: err.Error()})
				return
			}
			...
			if stopReason != "max_tokens" && stopReason != "length" {
				emit(cancelCtx, outCh, FluxStreamEvent{Type: "done", StopReason: stopReason})
				return
			}
			if hadToolCalls {
				emit(cancelCtx, outCh, FluxStreamEvent{Type: "done", StopReason: stopReason})
			...
			if cfg.MaxTotalTokens > 0 && int(totalCompletionTokens) >= cfg.MaxTotalTokens {
				emit(cancelCtx, outCh, FluxStreamEvent{Type: "done", StopReason: "max_tokens"})
				return
			}
			...
			emit(cancelCtx, outCh, FluxStreamEvent{
				Type:       "continuation",
				Content:    groupID,
