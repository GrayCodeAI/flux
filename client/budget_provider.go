...
var ErrBudgetExceeded = errors.New("flux: virtual key budget exceeded")

// ErrUnknownVirtualKey is returned when a request references a virtual key that
// the store does not recognize.
var ErrUnknownVirtualKey = errors.New("flux: unknown virtual key")
...
func (bp *BudgetProvider) Chat(ctx context.Context, messages []FluxMessage, opts ChatOptions) (*FluxResponse, error) {
...
func (bp *BudgetProvider) StreamChat(ctx context.Context, messages []FluxMessage, opts ChatOptions) (*StreamResult, error) {
	vk := resolveVirtualKey(ctx, opts)
	if vk == "" || bp.store == nil {
...
func ActualCostUSD(model string, usage *FluxUsage) float64 {
	if usage == nil {
		return 0
	}
