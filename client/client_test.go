...
func TestParseCustomHeaders(t *testing.T) {
	_ = os.Setenv("FLUX_CUSTOM_HEADERS", "X-Custom: value1\nX-Other: value2")
	defer func() { _ = os.Unsetenv("FLUX_CUSTOM_HEADERS") }()
...
func TestClient(t *testing.T) {
	c := Client(&FluxConfig{Provider: "openai", APIKey: "test-key"})
...
func TestClientConfigBaseURL_OpenAICompatible(t *testing.T) {
	c := Client(&FluxConfig{Provider: "openrouter", APIKey: "test-key", BaseURL: "https://proxy.example/v1"})
...
func TestClientConfigBaseURL_Anthropic(t *testing.T) {
	c := Client(&FluxConfig{Provider: "anthropic", APIKey: "test-key", BaseURL: "https://anthropic-proxy.example"})
...
func TestAnthropicClientChat(t *testing.T) {
	...
	resp, err := ac.Chat(context.Background(), []FluxMessage{
...
func TestOpenAIClientChat(t *testing.T) {
	...
	resp, err := oc.Chat(context.Background(), []FluxMessage{
...
func TestStreamParsing(t *testing.T) {
	...
	oc := NewOpenAIClient("test-key", server.URL, &OpenAICompat)
	sr, err := oc.StreamChat(context.Background(), []FluxMessage{
		{Role: "user", Content: "Hi"},
...
func TestFluxClientEmptyMessages(t *testing.T) {
	c := Client(&FluxConfig{Provider: "openai", APIKey: "test"})
