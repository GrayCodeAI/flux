...
func (c *Coalescer) Coalesce(ctx context.Context, key CoalesceKey, fn func() (*FluxResponse, error)) (*FluxResponse, error) {
	keyStr := key.String()
	...
	if existing, ok := c.inflight[keyStr]; ok {
		...
		if existing.waiters >= maxCoalesceWaiters {
			existing.mu.Unlock()
			c.mu.Unlock()
			return nil, fmt.Errorf("flux: coalesce limit reached (%d waiters); retry independently", maxCoalesceWaiters)
		}
		existing.waiters++
		existing.mu.Unlock()
		c.mu.Unlock()
...
func (r *InflightRequest) wait(ctx context.Context) (*FluxResponse, error) {
	select {
	case <-ctx.Done():
		return nil, ctx.Err()
	case <-r.ctx.Done():
