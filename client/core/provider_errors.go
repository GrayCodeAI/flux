...
// ProviderErrorDetail holds the structured fields flux can extract from a
...
// so callers can attach it to the structured *FluxError.
...
// table, to avoid becoming a maintenance sink across flux's many providers.
...
// FormatAPIError builds the *FluxError used across every provider
...
//
// Returning *FluxError (rather than a plain fmt.Errorf) lets
...
// could not be read). It is attached via FluxError.Err so
