...
func (e *GuardrailError) Error() string {
	return fmt.Sprintf("flux: guardrail blocked: %s (%d violation(s))", e.Message, len(e.Violations))
}
...
func (g *Guardrails) AddRule(r GuardrailRule) {
	if r.Pattern != "" {
		...
		if err != nil {
			panic(fmt.Sprintf("flux: guardrails: invalid regex %q in rule %q: %v", r.Pattern, r.Name, err))
		}
		r.compiled = compiled
	}
	g.mu.Lock()
...
func (g *Guardrails) AddRuleSafe(r GuardrailRule) error {
	if r.Pattern != "" {
		compiled, err := regexp.Compile(r.Pattern)
		if err != nil {
			return fmt.Errorf("flux: guardrails: invalid regex %q in rule %q: %w", r.Pattern, r.Name, err)
		}
		r.compiled = compiled
	}
	g.mu.Lock()
...
func ApplyGuardrails(ctx context.Context, resp *FluxResponse, g *Guardrails) error {
