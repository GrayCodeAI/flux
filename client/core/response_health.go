...
// ResponseHealth classifies the outcome of a model response so flux can turn a
...
func (h ResponseHealth) Err() error {
	if d := h.Diagnostic(); d != "" {
		return fmt.Errorf("flux: %s: %s", h, d)
	}
	return nil
}

// healthFromResponse classifies a non-streaming FluxResponse. Because the
...
func healthFromResponse(resp *FluxResponse, sawReasoning bool) ResponseHealth {
	if resp == nil {
		return ResponseEmpty
	}
