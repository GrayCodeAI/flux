...
func TestGeminiClient_Chat_Success(t *testing.T) {
	...
	resp, err := c.Chat(context.Background(), []core.FluxMessage{{Role: "user", Content: "Hi"}}, core.ChatOptions{Model: "gemini-2.0-flash", MaxTokens: 256})
	if err != nil {
		t.Fatalf("Chat: %v", err)
	}
...
func TestGeminiClient_Chat_APIError(t *testing.T) {
	...
	c.retry = core.RetryConfig{RetryConfig: types.RetryConfig{MaxRetries: 0}}
	c.httpClient = &http.Client{Transport: transport}
	_, err := c.Chat(context.Background(), []core.FluxMessage{{Role: "user", Content: "Hi"}}, core.ChatOptions{Model: "gemini-2.0-flash", MaxTokens: 256})
	if err == nil {
...
func TestGeminiClient_StreamChat_Success(t *testing.T) {
	...
	c.retry = core.RetryConfig{RetryConfig: types.RetryConfig{MaxRetries: 0}}
	c.httpClient = &http.Client{Transport: transport}
	result, err := c.StreamChat(context.Background(), []core.FluxMessage{{Role: "user", Content: "Hi"}}, core.ChatOptions{Model: "gemini-2.0-flash", MaxTokens: 256})
	if err != nil {
...
func TestGeminiClient_StreamChat_PropagatesRequestID(t *testing.T) {
	...
	c.retry = core.RetryConfig{RetryConfig: types.RetryConfig{MaxRetries: 0}}
	c.httpClient = &http.Client{Transport: transport}
	result, err := c.StreamChat(context.Background(), []core.FluxMessage{{Role: "user", Content: "Hi"}}, core.ChatOptions{Model: "gemini-2.0-flash"})
	if err != nil {
		t.Fatalf("StreamChat: %v", err)
	}
	defer result.Close()
...
func TestGeminiClient_Chat_WithSystem(t *testing.T) {
	...
	c.retry = core.RetryConfig{RetryConfig: types.RetryConfig{MaxRetries: 0}}
	c.httpClient = &http.Client{Transport: transport}
	resp, err := c.Chat(context.Background(), []core.FluxMessage{
...
func TestGeminiClient_Chat_WithTools(t *testing.T) {
	...
	c.retry = core.RetryConfig{RetryConfig: types.RetryConfig{MaxRetries: 0}}
	c.httpClient = &http.Client{Transport: transport}
	resp, err := c.Chat(context.Background(), []core.FluxMessage{{Role: "user", Content: "Weather?"}}, core.ChatOptions{
		Model: "gemini-2.0-flash", MaxTokens: 256,
		Tools:      []core.FluxTool{{Name: "get_weather", Description: "Get weather", Parameters: map[string]interface{}{"type": "object"}}},
...
func TestGeminiClient_Chat_WithResponseFormat(t *testing.T) {
	...
	c.retry = core.RetryConfig{RetryConfig: types.RetryConfig{MaxRetries: 0}}
	c.httpClient = &http.Client{Transport: transport}
	resp, err := c.Chat(context.Background(), []core.FluxMessage{{Role: "user", Content: "JSON please"}}, core.ChatOptions{
		Model: "gemini-2.0-flash", MaxTokens: 256,
...
func TestGeminiClient_Chat_AssistantRole(t *testing.T) {
	...
	c.retry = core.RetryConfig{RetryConfig: types.RetryConfig{MaxRetries: 0}}
	c.httpClient = &http.Client{Transport: transport}
	_, err := c.Chat(context.Background(), []core.FluxMessage{{Role: "assistant", Content: "Hello"}}, core.ChatOptions{Model: "gemini-2.0-flash", MaxTokens: 256})
	if err != nil {
		t.Fatalf("Chat: %v", err)
	}
}
...
func TestGeminiClient_Chat_UnknownRole(t *testing.T) {
	...
	c.retry = core.RetryConfig{RetryConfig: types.RetryConfig{MaxRetries: 0}}
	c.httpClient = &http.Client{Transport: transport}
	_, err := c.Chat(context.Background(), []core.FluxMessage{{Role: "function", Content: "result"}}, core.ChatOptions{Model: "gemini-2.0-flash", MaxTokens: 256})
	if err != nil {
		t.Fatalf("Chat: %v", err)
	}
}
...
func TestGeminiClient_Chat_ToolChoiceNone(t *testing.T) {
	...
	_, err := c.Chat(context.Background(), []core.FluxMessage{{Role: "user", Content: "Hi"}}, core.ChatOptions{
		...
		Tools:      []core.FluxTool{{Name: "test", Description: "a test tool"}},
	})
	if err != nil {
		t.Fatalf("Chat: %v", err)
	}
...
func TestProcessStreamChunk_Content(t *testing.T) {
	...
	events := make(chan core.FluxStreamEvent, 1)
...
func TestProcessStreamChunk_ContextCancelled(t *testing.T) {
	t.Parallel()
	c := NewGeminiClient("key", "https://gemini.example")
	events := make(chan core.FluxStreamEvent)
