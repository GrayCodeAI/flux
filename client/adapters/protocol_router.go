...
type streamOpener func(context.Context, []core.GraycodeRouterMessage, core.ChatOptions) (*core.StreamResult, error)

type chatOpener func(context.Context, []core.GraycodeRouterMessage, core.ChatOptions) (*core.GraycodeRouterResponse, error)
...
func (r ProtocolRouter) Chat(ctx context.Context, messages []core.GraycodeRouterMessage, opts core.ChatOptions, primary ChatProtocol, fallback ProtocolChatFallback) (*core.GraycodeRouterResponse, error) {
...
// chat/completions.
func (r ProtocolRouter) StreamChat(ctx context.Context, messages []core.GraycodeRouterMessage, opts core.ChatOptions, cfg ProtocolStreamConfig) (*core.StreamResult, error) {
	...
	if cfg.ReasoningOnlyFallback && cfg.Primary == ChatProtocolMessages {
		...
		return newStreamWithReasoningFallback(ctx, messages, opts, result, protocolStreamFallback{
			chat: func(ctx context.Context, messages []core.GraycodeRouterMessage, opts core.ChatOptions) (*core.GraycodeRouterResponse, error) {
				return fallback.Chat(ctx, messages, opts)
			},
			stream: func(ctx context.Context, messages []core.GraycodeRouterMessage, opts core.ChatOptions) (*core.StreamResult, error) {
...
func (f protocolStreamFallback) open(ctx context.Context, messages []core.GraycodeRouterMessage, opts core.ChatOptions) (*core.StreamResult, error) {
	if f.chat != nil {
	...
	return nil, fmt.Errorf("graycode-router: protocol stream fallback is not configured")
}

func newStreamWithReasoningFallback(ctx context.Context, messages []core.GraycodeRouterMessage, opts core.ChatOptions, primary *core.StreamResult, fallback protocolStreamFallback) *core.StreamResult {
	...
	go func() {
		...
		var (
			...
			toolCalls    int
			buffered     []core.GraycodeRouterStreamEvent
			streamErr    bool
		)
		flush := func() {
		...
		if err != nil {
			flush()
			select {
			case out <- core.GraycodeRouterStreamEvent{Type: "error", Error: err.Error()}:
			case <-cancelCtx.Done():
			}
			return
		}
