...
// DynamicProviderEnvVar is the opt-in env var that allows flux to
...
const DynamicProviderEnvVar = "FLUX_ALLOW_DYNAMIC_PROVIDERS"
...
func RegisterDynamicProvider(name, baseURL, envKey string) error {
	if registryFrozen.Load() {
		return fmt.Errorf("flux: provider registry is frozen; register providers before first use")
	}
	if baseURL == "" {
		return fmt.Errorf("flux: RegisterDynamicProvider: baseURL must not be empty")
	}
	u, err := url.Parse(baseURL)
	if err != nil || (u.Scheme != "http" && u.Scheme != "https") || u.Host == "" {
		return fmt.Errorf("flux: RegisterDynamicProvider: invalid baseURL %q (must be http/https with host)", baseURL)
	}
	DynamicMu.Lock()
