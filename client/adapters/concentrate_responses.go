...
func (c *ConcentrateResponsesClient) Chat(ctx context.Context, messages []core.FluxMessage, opts core.ChatOptions) (*core.FluxResponse, error) {
	...
	httpReq.Header.Set("User-Agent", "flux-model-catalog/1.0")
	...
	return c.toFluxResponse(apiResp), nil
}

// StreamChat implements core.Provider using the Responses API with SSE streaming.
func (c *ConcentrateResponsesClient) StreamChat(ctx context.Context, messages []core.FluxMessage, opts core.ChatOptions) (*core.StreamResult, error) {
...
func (c *ConcentrateResponsesClient) Ping(ctx context.Context) error {
	...
	req.Header.Set("User-Agent", "flux-model-catalog/1.0")
	if c.apiKey != "" {
...
func (c *ConcentrateResponsesClient) buildRequest(messages []core.FluxMessage, opts core.ChatOptions, stream bool) (responsesRequest, error) {
...
func (c *ConcentrateResponsesClient) messagesToInput(messages []core.FluxMessage) []map[string]interface{} {
...
func (c *ConcentrateResponsesClient) toFluxResponse(resp responsesResponse) *core.FluxResponse {
	fluxResp := &core.FluxResponse{
	...
	for _, item := range resp.Output {
		if item.Type == "function_call" {
			...
			fluxResp.ToolCalls = append(fluxResp.ToolCalls, core.ToolCall{
				ID:        item.CallID,
				Name:      item.Name,
				Arguments: args,
			})
...
func (c *ConcentrateResponsesClient) handleStream(ctx context.Context, cancel context.CancelFunc, resp *http.Response, requestID string) *core.StreamResult {
	events := make(chan core.FluxStreamEvent, core.StreamChannelBuffer)
	...
	go func() {
		...
		for {
			...
			if err != nil {
				if err != io.EOF {
					sendConcentrateStreamEvent(ctx, events, core.FluxStreamEvent{Type: "error", Error: err.Error()})
				}
				return
			}
			...
			case "response.output_text.delta":
				if event.Delta != "" {
					if !sendConcentrateStreamEvent(ctx, events, core.FluxStreamEvent{Type: "content", Content: event.Delta}) {
						return
					}
				}
			case "response.reasoning_text.delta", "response.reasoning_summary_text.delta":
				if event.Delta != "" {
					if !sendConcentrateStreamEvent(ctx, events, core.FluxStreamEvent{Type: "thinking", Thinking: event.Delta}) {
						return
					}
				}
			...
			case "response.function_call_arguments.done":
				...
				if !sendConcentrateStreamEvent(ctx, events, core.FluxStreamEvent{
					Type: "tool_call",
			...
			case "response.completed":
				...
				if err := json.Unmarshal(event.Response, &r); err == nil && r.Usage != nil {
					if !sendConcentrateStreamEvent(ctx, events, core.FluxStreamEvent{Type: "usage", Usage: &core.FluxUsage{
				...
				sendConcentrateStreamEvent(ctx, events, core.FluxStreamEvent{Type: "done", StopReason: stopReason})
				return
			case "response.incomplete":
				...
				sendConcentrateStreamEvent(ctx, events, core.FluxStreamEvent{Type: "done", StopReason: concentrateFinishReason(r)})
				return
			case "response.failed":
				...
				sendConcentrateStreamEvent(ctx, events, core.FluxStreamEvent{Type: "error", Error: message})
				return
			case "error":
...
func sendConcentrateStreamEvent(ctx context.Context, events chan<- core.FluxStreamEvent, event core.FluxStreamEvent) bool {
	select {
	case events <- event:
		return true
	case <-ctx.Done():
