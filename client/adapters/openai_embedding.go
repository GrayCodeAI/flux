...
func (c *OpenAIClient) CreateEmbedding(ctx context.Context, req core.EmbeddingRequest) (*core.EmbeddingResponse, error) {
	if req.Model == "" {
		return nil, fmt.Errorf("flux: model is required for %s embeddings", c.providerName)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("flux: failed to marshal embedding request: %w", err)
	}

	httpReq, err := http.NewRequestWithContext(ctx, "POST", c.baseURL+"/embeddings", bytes.NewReader(body))
	if err != nil {
		return nil, fmt.Errorf("flux: failed to create embedding request: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("flux: %s embedding request failed: %w", c.providerName, err)
	}
	defer func() {
	...
	if err := json.NewDecoder(resp.Body).Decode(&or); err != nil {
		return nil, fmt.Errorf("flux: failed to decode %s embedding response: %w", c.providerName, err)
	}
