...
func TestNewOpenRouterClient_OpenAI(t *testing.T) {
	t.Parallel()
	client := NewOpenRouterClient("or-key", "https://openrouter.ai/api/v1", &OpenRouterCompat)
...
func TestOpenRouterClient_ChatUsesOpenAIPath(t *testing.T) {
	t.Parallel()
	var path string
	...
	resp, err := client.Chat(context.Background(), []core.FluxMessage{{Role: "user", Content: "hi"}}, core.ChatOptions{Model: "anthropic/claude", MaxTokens: 16})
	if err != nil {
		t.Fatal(err)
	}
...
func TestOpenRouterClient_Ping(t *testing.T) {
	t.Parallel()
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	defer server.Close()
	client := NewOpenRouterClient("key", server.URL, &OpenRouterCompat, core.WithTimeout(2*time.Second))
