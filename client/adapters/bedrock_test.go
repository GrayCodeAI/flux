...
func TestBedrockClient_Chat_Success(t *testing.T) {
	...
	resp, err := c.Chat(context.Background(), []core.FluxMessage{{Role: "user", Content: "Hi"}}, core.ChatOptions{Model: "anthropic.claude-sonnet-4-20250514", MaxTokens: 256})
	if err != nil {
		t.Fatalf("Chat: %v", err)
	}
...
func TestBedrockClient_Chat_EmptyRegion(t *testing.T) {
	t.Parallel()
	c := NewBedrockClient("", "", "", "")
	_, err := c.Chat(context.Background(), []core.FluxMessage{{Role: "user", Content: "Hi"}}, core.ChatOptions{Model: "claude", MaxTokens: 256})
	if err == nil {
...
func TestBedrockClient_BuildBody(t *testing.T) {
	t.Parallel()
	c := NewBedrockClient("AKID", "secret", "", "us-east-1")
	body, err := c.BuildBody([]core.FluxMessage{{Role: "user", Content: "hi"}}, core.ChatOptions{Model: "claude", MaxTokens: 256})
	if err != nil {
...
func TestBedrockClient_StreamChat_Success(t *testing.T) {
	...
	result, err := c.StreamChat(context.Background(), []core.FluxMessage{{Role: "user", Content: "Hi"}}, core.ChatOptions{Model: "anthropic.claude-sonnet-4-20250514", MaxTokens: 256})
	if err != nil {
...
func TestBedrockClient_StreamChat_Error(t *testing.T) {
	...
	c.retry = core.RetryConfig{RetryConfig: types.RetryConfig{MaxRetries: 0}}
	c.httpClient = &http.Client{Transport: transport}
	_, err := c.StreamChat(context.Background(), []core.FluxMessage{{Role: "user", Content: "Hi"}}, core.ChatOptions{Model: "claude", MaxTokens: 256})
	if err == nil {
		t.Fatal("expected error")
	}
}
...
func TestBedrockClient_Chat_DefaultMaxTokens(t *testing.T) {
	...
	c.retry = core.RetryConfig{RetryConfig: types.RetryConfig{MaxRetries: 0}}
	c.httpClient = &http.Client{Transport: transport}
	_, err := c.Chat(context.Background(), []core.FluxMessage{{Role: "user", Content: "Hi"}}, core.ChatOptions{Model: "anthropic.claude-sonnet-4-20250514"})
	if err != nil {
		t.Fatalf("Chat: %v", err)
	}
}
...
func TestBedrockClient_Chat_SystemOnlyOpts(t *testing.T) {
	...
	c.retry = core.RetryConfig{RetryConfig: types.RetryConfig{MaxRetries: 0}}
	c.httpClient = &http.Client{Transport: transport}
	_, err := c.Chat(context.Background(), []core.FluxMessage{{Role: "user", Content: "Hi"}}, core.ChatOptions{Model: "anthropic.claude-sonnet-4-20250514", MaxTokens: 256, System: "only from opts"})
	if err != nil {
		t.Fatalf("Chat: %v", err)
	}
}
