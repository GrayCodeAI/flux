...
func (bc *BatchClient) WaitUntilDone(ctx context.Context, batchID string, opts PollOptions) (*BatchResult, error) {
	...
	for {
		...
		if err != nil {
			return nil, fmt.Errorf("flux: batch wait: %w", err)
		}
		switch {
		...
		default:
			...
			_ = resp.Body.Close()
			return nil, fmt.Errorf("flux: batch wait error %d: %s", resp.StatusCode, strings.TrimSpace(string(errBody)))
		}

		if time.Now().After(deadline) {
			return nil, fmt.Errorf("flux: batch %s not terminal within %s", batchID, opts.Timeout)
		}
...
var ErrResultNotVisible = errors.New("flux: batch result row not yet visible")
...
func (bc *BatchClient) RequestResults(ctx context.Context, batchID string) ([]BatchRequestResult, error) {
	...
	resp, err := bc.httpClient.Do(req)
	if err != nil {
		return nil, fmt.Errorf("flux: batch results: %w", err)
	}
	defer func() {
	...
	if resp.StatusCode != http.StatusOK {
		errBody, _ := io.ReadAll(io.LimitReader(resp.Body, 4096))
		return nil, fmt.Errorf("flux: batch results error %d: %s", resp.StatusCode, strings.TrimSpace(string(errBody)))
	}
	...
	for scanner.Scan() {
		...
		if err := json.Unmarshal(line, &r); err != nil {
			return nil, fmt.Errorf("flux: parse results line: %w", err)
		}
		out = append(out, r)
	}
