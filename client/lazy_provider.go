...
// LazyProvider adapts FluxClient to the Provider interface without eagerly
...
func NewLazyProvider(cfg *FluxConfig) *LazyProvider {
	c := Client(cfg)
...
func (p *LazyProvider) Chat(ctx context.Context, messages []FluxMessage, opts ChatOptions) (*FluxResponse, error) {
	if opts.Provider == "" {
...
func (p *LazyProvider) StreamChat(ctx context.Context, messages []FluxMessage, opts ChatOptions) (*StreamResult, error) {
	if opts.Provider == "" {
		opts.Provider = p.provider
	}
