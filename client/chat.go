...
func (c *FluxClient) Chat(ctx context.Context, messages []FluxMessage, opts ChatOptions) (*FluxResponse, error) {
	if len(messages) == 0 {
	...
	if c.coalescer != nil {
		...
		return c.coalescer.Coalesce(ctx, key, func() (*FluxResponse, error) {
...
func (c *FluxClient) StreamChat(ctx context.Context, messages []FluxMessage, opts ChatOptions) (*StreamResult, error) {
	if len(messages) == 0 {
		return nil, fmt.Errorf("flux: messages must not be empty")
	}
	provider := opts.Provider
...
func (c *FluxClient) StreamChatContinue(ctx context.Context, messages []FluxMessage, opts ChatOptions, cfg ContinuationConfig) (*StreamResult, error) {
	if len(messages) == 0 {
		return nil, fmt.Errorf("flux: messages must not be empty")
	}
	provider := opts.Provider
