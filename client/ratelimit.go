...
func (b *tokenBucket) wait(ctx context.Context) error {
	for {
		...
		select {
		case <-ctx.Done():
			return fmt.Errorf("flux: rate limiter: %w", ctx.Err())
		default:
		}
...
func (r *RateLimitedProvider) Chat(ctx context.Context, messages []FluxMessage, opts ChatOptions) (*FluxResponse, error) {
...
func (r *RateLimitedProvider) StreamChat(ctx context.Context, messages []FluxMessage, opts ChatOptions) (*StreamResult, error) {
	if err := r.limiter.Wait(ctx, r.inner.Name()); err != nil {
		return nil, err
	}
