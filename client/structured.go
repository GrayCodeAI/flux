...
func BuildStructuredPrompt(messages []FluxMessage, schema map[string]interface{}) []FluxMessage {
...
- All required fields must be present`, string(schemaJSON))
	...
	result := make([]FluxMessage, 0, len(messages)+1)
	systemFound := false
	...
	if !systemFound {
		...
		systemMsg := FluxMessage{Role: "system", Content: schemaInstruction}
		result = append([]FluxMessage{systemMsg}, result...)
	}
...
func (c *FluxClient) ChatWithStructuredOutput(ctx context.Context, messages []FluxMessage, opts ChatOptions, validation SchemaValidation) (*FluxResponse, error) {
	if len(messages) == 0 {
...
func addAnthropicPrefill(messages []FluxMessage) []FluxMessage {
	// Add assistant message with opening brace to force JSON output
	prefillMsg := FluxMessage{
		Role:    "assistant",
		Content: "```json\n{",
	}
...
func addRetryFeedback(messages []FluxMessage, lastResponse string, validationErr error, schema map[string]interface{}) []FluxMessage {
