...
func (m *MockProvider) StreamChat(ctx context.Context, messages []FluxMessage, opts ChatOptions) (*StreamResult, error) {
	...
	go func() {
		...
		if resp.FinishReason == "tool_use" && len(resp.ToolCalls) > 0 {
			emit(streamCtx, ch, FluxStreamEvent{Type: "tool_call", ToolCall: &resp.ToolCalls[0]})
		} else {
			...
			for _, w := range words {
				...
				emit(streamCtx, ch, FluxStreamEvent{Type: "content", Content: w + " "})
			}
		}
		emit(streamCtx, ch, FluxStreamEvent{Type: "done"})
	}()
