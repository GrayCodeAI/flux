...
func TestGetOrCreateProvider_VertexUsesAnthropicVertexClient(t *testing.T) {
	...
	c := Client(&FluxConfig{Provider: "vertex", APIKey: "test-bearer-token"})
...
func TestGetOrCreateProvider_VertexRegionDefaultsToUsCentral1(t *testing.T) {
	...
	c := Client(&FluxConfig{Provider: "vertex", APIKey: "test-token"})
	p, err := c.getOrCreateProvider("vertex")
	if err != nil {
		t.Fatalf("getOrCreateProvider: %v", err)
	}
...
func TestGetOrCreateProvider_VertexRequiresProjectID(t *testing.T) {
	...
	if got := err.Error(); got != "flux: vertex requires VERTEX_PROJECT_ID" {
		t.Errorf("error = %q, want %q", got, "flux: vertex requires VERTEX_PROJECT_ID")
	}
}

// TestDynamicProvider_DefaultDeny: when FLUX_ALLOW_DYNAMIC_PROVIDERS is
...
// TestDynamicProvider_OptIn_Registers: with FLUX_ALLOW_DYNAMIC_PROVIDERS=1
