# Design Doc: Flux Enterprise Gateway
...
Flux today is a privacy-first, zero-cloud, Go-static-binary LLM provider runtime. It already
...
This doc designs the **Flux Enterprise Gateway** as a cohesive surface built *on top of*
existing flux packages, preserving the privacy-first posture (local-first, hash-only audit,
no telemetry exfiltration by default).

### Which Top-20 repos ship these features

From `TOP20_COMPARISON.md` (the flux section, lines 89–124):
...
Flux's gap summary in the comparison doc: **2 P0 / 13 P1 / 5 P2** (`TOP20_COMPARISON.md:271`).
...
3. **Analytics dashboard UI** (Helicone-style): per-request segmentation, session tracking,
   ...
   21+ metrics flux's `MetricsCollector` already computes.
...
6. **A2A protocol** so rho agents can call external agents (LangGraph, Vertex Agent Engine,
   Azure AI Foundry, Bedrock AgentCore) as tool calls through the flux proxy.
7. **Fine-tuning workflow client** that submits training data, polls jobs, and registers the
   resulting model in flux's catalog.
8. **Priority queue with SLA tiers** at the rate-limit layer: interactive traffic preempts
   batch/background traffic.

### Non-Goals

- **Flux does not become a hosted SaaS.** The enterprise gateway is a self-hostable binary +
...
- The browser/IDE/cloud-execution gaps belong to rho, not this doc.
...
GET    /auth/callback           -> exchange code, mint flux session cookie
POST   /auth/logout
...
## 4. Integration With Existing Flux Code (what is reusable today)
...
Maps to the two flux **P0** comparison gaps (`TOP20_COMPARISON.md:96`) plus the analytics-UI
...
| Dashboard UI | **Build** a small bundle, `go:embed` into the binary | Keeps flux a single static binary (its core differentiator). Consider Preact/Svelte (MIT) compiled to a static bundle — no runtime server-side framework. |
...
**Licensing:** flux is MIT (`LICENSE`, "Copyright (c) 2026 Rho Contributors"). All proposed
