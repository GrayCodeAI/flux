...
Single source: `flux/catalog/registry/providers.go`
...
Flux stores Anthropic **base** as `…/anthropic` (no `/v1`); `AnthropicClient` appends `/v1/messages`, matching [Anthropic API](https://platform.xiaomimimo.com/docs/en-US/api/chat/anthropic-api) and the Python SDK `base_url="https://api.xiaomimimo.com/anthropic"`.
...
Setup is **gateway-first**: pick the gateway on the Gateways tab, paste any non-empty secret (min length 8; placeholders rejected). Flux does **not** validate or infer provider from key prefixes (`sk-ant-`, `tp-`, etc.). Live probe runs only for the selected gateway.
