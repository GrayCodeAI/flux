...
func (gp *GuardrailProvider) Chat(ctx context.Context, messages []FluxMessage, opts ChatOptions) (*FluxResponse, error) {
	resp, err := gp.inner.Chat(ctx, messages, opts)
	if err != nil {
		return nil, err
	}
	if err := core.ApplyGuardrails(ctx, resp, gp.guardrails); err != nil {
		return nil, err
	}
	return resp, nil
}
...
func (gp *GuardrailProvider) StreamChat(ctx context.Context, messages []FluxMessage, opts ChatOptions) (*StreamResult, error) {
