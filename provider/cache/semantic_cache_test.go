...
func newCacheMock(mode string) *cacheMock       { return &cacheMock{mode: mode} }
func (m *cacheMock) Name() string               { return "mock" }
func (m *cacheMock) Ping(context.Context) error { return nil }
func (m *cacheMock) CallCount() int {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.calls
}
	...
	if m.mode == "echo" {
		for i := len(messages) - 1; i >= 0; i-- {
			if messages[i].Role == "user" {
				content = "echo: " + messages[i].Content
				break
			}
		}
	}
...
func TestCachedProviderDifferentInputsMiss(t *testing.T) {
	t.Parallel()
	inner := newCacheMock("echo")

	cp := NewCachedProvider(inner, DefaultCacheConfig())

	// Two different messages should produce two different cache keys.
	resp1, err := cp.Chat(context.Background(), []FluxMessage{
	...
	resp2, err := cp.Chat(context.Background(), []FluxMessage{
...
func TestCachedProviderLRUEviction(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		_, err := cp.Chat(context.Background(), []FluxMessage{
