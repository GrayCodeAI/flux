...
// formatAPIError constructs *FluxError. Structured-error awareness
// lives in the fallback chain (fallback.go:240-244) where
// *FluxError.IsRetriable() / IsAuthError() drive provider
...
func DoWithRetry(ctx context.Context, httpClient *http.Client, req *http.Request, rc RetryConfig, logger *slog.Logger) (*http.Response, error) {
	...
	return nil, fmt.Errorf("flux: %s: max retries (%d) exceeded for %s: %w", req.URL.Host, rc.MaxRetries, req.URL.Path, lastErr)
}
