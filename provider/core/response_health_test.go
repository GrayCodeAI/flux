...
func TestHealthFromResponse(t *testing.T) {
	t.Parallel()
	if got := healthFromResponse(&FluxResponse{Content: "hello"}, false); got != ResponseOK {
		t.Errorf("got %q, want ok", got)
	}
	if got := healthFromResponse(&FluxResponse{}, true); got != ResponseErrorOnlyReasoning {
		t.Errorf("got %q, want error_only_reasoning", got)
	}
	if got := healthFromResponse(&FluxResponse{}, false); got != ResponseEmpty {
