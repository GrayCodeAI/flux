...
func TestModerationProvider_AllowsSafe(t *testing.T) {
	...
	mp := resilience.NewModerationProvider(
		mock,
		resilience.WithBlockedPatterns([]string{"(?i)forbidden"}),
		resilience.WithModerationMaxTokens(1000),
	)

	msgs := []FluxMessage{{Role: "user", Content: "Hello, how are you?"}}
...
func TestModerationProvider_BlocksPattern(t *testing.T) {
	...
	mp := resilience.NewModerationProvider(
		mock,
		resilience.WithBlockedPatterns([]string{`(?i)\bforbidden\b`}),
	)

	msgs := []FluxMessage{{Role: "user", Content: "This contains forbidden content"}}
...
func TestModerationProvider_TokenLimit(t *testing.T) {
	...
	mp := resilience.NewModerationProvider(
		mock,
		resilience.WithModerationMaxTokens(5),
	)

	// 10 words exceeds limit of 5
	msgs := []FluxMessage{{Role: "user", Content: "one two three four five six seven eight nine ten"}}
	_, err := mp.Chat(context.Background(), msgs, ChatOptions{Model: "test"})
	if err == nil {
...
func TestModerationProvider_TokenLimitAllowsUnderLimit(t *testing.T) {
	...
	mp := resilience.NewModerationProvider(
		mock,
		resilience.WithModerationMaxTokens(100),
	)

	msgs := []FluxMessage{{Role: "user", Content: "short message"}}
	_, err := mp.Chat(context.Background(), msgs, ChatOptions{Model: "test"})
	if err != nil {
		t.Fatalf("expected no error, got: %v", err)
	}
...
func TestModerationProvider_CustomChecker(t *testing.T) {
	...
	mp := resilience.NewModerationProvider(
		mock,
		resilience.WithCustomChecker(func(text string) error {
	...
	msgs := []FluxMessage{{Role: "user", Content: "This has a Banned word"}}
	_, err := mp.Chat(context.Background(), msgs, ChatOptions{Model: "test"})
	if err == nil {
...
func TestModerationProvider_CustomCheckerAllows(t *testing.T) {
	...
	msgs := []FluxMessage{{Role: "user", Content: "safe content"}}
	_, err := mp.Chat(context.Background(), msgs, ChatOptions{Model: "test"})
	if err != nil {
		t.Fatalf("expected no error, got: %v", err)
	}
...
func TestModerationProvider_StreamChatBlocked(t *testing.T) {
	...
	msgs := []FluxMessage{{Role: "user", Content: "forbidden content"}}
...
func TestModerationProvider_ContentParts(t *testing.T) {
	...
	mp := resilience.NewModerationProvider(
		mock,
		resilience.WithBlockedPatterns([]string{`(?i)secret`}),
	)
...
func TestModerationProvider_Name(t *testing.T) {
	t.Parallel()
	mock := NewMockProvider(MockModeEcho)
	mp := resilience.NewModerationProvider(mock)
...
func TestModerationProvider_NilInner(t *testing.T) {
	t.Parallel()
	mp := resilience.NewModerationProvider(nil)
	if mp != nil {
