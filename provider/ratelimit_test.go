...
func TestRateLimitProviderDelegation(t *testing.T) {
	...
	rl := resilience.NewRateLimiter(resilience.RateLimitConfig{RequestsPerMinute: 600, BurstSize: 10})
	wrapped := resilience.WithRateLimit(mock, rl)
...
func TestRateLimitChatBlockedByContext(t *testing.T) {
	...
	rl := resilience.NewRateLimiter(resilience.RateLimitConfig{RequestsPerMinute: 60, BurstSize: 1})
	wrapped := resilience.WithRateLimit(mock, rl)
