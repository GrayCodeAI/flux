...
// StructuredChatter is the narrow contract extraction needs from a provider
// runtime. It keeps this feature independent of provider.FluxClient.
type StructuredChatter interface {
	ChatWithStructuredOutput(context.Context, []core.FluxMessage, core.ChatOptions, core.SchemaValidation) (*core.FluxResponse, error)
}
...
func ExtractRelationships(ctx context.Context, c StructuredChatter, text string, opts ExtractOptions) ([]Relationship, error) {
	if c == nil {
		return nil, fmt.Errorf("flux: extract: structured chatter must not be nil")
	}
	if strings.TrimSpace(text) == "" {
		return nil, fmt.Errorf("flux: extract: text must not be empty")
	}
	...
	resp, err := c.ChatWithStructuredOutput(ctx, messages, opts.Chat, core.SchemaValidation{
		Schema:     schema,
		MaxRetries: maxRetries,
	})
	if err != nil {
		return nil, fmt.Errorf("flux: extract relationships: %w", err)
	}
	...
	if err := json.Unmarshal([]byte(resp.Content), &parsed); err != nil {
		return nil, fmt.Errorf("flux: extract relationships: decode: %w", err)
	}
