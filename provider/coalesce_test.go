...
func TestCoalesceDeduplicatesIdenticalRequests(t *testing.T) {
	...
	coalescer := resilience.NewCoalescer(100 * time.Millisecond)
...
func TestCoalesceDifferentKeysNotDeduplicated(t *testing.T) {
	...
	for i, key := range keys {
		wg.Add(1)
		go func(idx int, k resilience.CoalesceKey) {
			defer wg.Done()
...
func TestCoalesceStats(t *testing.T) {
	...
	waitForStats(t, coalescer, func(s resilience.InflightStats) bool {
...
func waitForStats(t *testing.T, c *resilience.Coalescer, pred func(resilience.InflightStats) bool) resilience.InflightStats {
	t.Helper()
