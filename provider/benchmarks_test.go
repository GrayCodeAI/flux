...
import (
	...
	providerobs "github.com/GrayCodeAI/flux/provider/observability"
)
...
func BenchmarkBuildCacheKey_Short(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = providercache.BuildCacheKey(messages, opts)
	}
}
...
func BenchmarkCachedProvider_CacheHit(b *testing.B) {
	...
	cp := providercache.NewCachedProvider(mock, providercache.DefaultCacheConfig())
	messages := []FluxMessage{{Role: "user", Content: "Hello"}}
	opts := ChatOptions{Model: "gpt-4"}
...
func BenchmarkCachedProvider_CacheMiss(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		messages := []FluxMessage{{Role: "user", Content: "unique query"}}
		_, _ = cp.Chat(context.Background(), messages, opts)
	}
}
...
func BenchmarkSanitizeMessages_Large(b *testing.B) {
	messages := make([]FluxMessage, 50)
	for i := range messages {
		if i%3 == 0 {
			messages[i] = FluxMessage{Role: "user", Content: "message"}
		} else {
			messages[i] = FluxMessage{Role: "assistant", Content: "response"}
		}
	}
	b.ReportAllocs()
	b.ResetTimer()
...
func BenchmarkMetricsCollector_Record(b *testing.B) {
	mc := providerobs.NewMetricsCollector()
	m := providerobs.CallMetrics{Model: "gpt-4", Provider: "openai", InputTokens: 100, OutputTokens: 50, LatencyMs: 100}
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		mc.Record(m)
...
func BenchmarkMetricsCollector_TotalCost(b *testing.B) {
	mc := providerobs.NewMetricsCollector()
	m := providerobs.CallMetrics{Model: "gpt-4", Provider: "openai", InputTokens: 1000, OutputTokens: 500, CacheReadTokens: 200, CacheCreationTokens: 100}
	for i := 0; i < 100; i++ {
		mc.Record(m)
	}
	b.ReportAllocs()
