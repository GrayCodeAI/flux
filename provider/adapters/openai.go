...
// NewOpenAIClient creates a configured OpenAI/compatible provider.
...
// a prior assistant turn (core.FluxMessage.Thinking) is forwarded back into assistant
...
func buildRequestBase(messages []core.FluxMessage, opts core.ChatOptions, stream bool, compat *OpenAICompatConfig) openaiRequest {
...
func (c *OpenAIClient) buildRequest(messages []core.FluxMessage, opts core.ChatOptions, stream bool) openaiRequest {
...
// header.
func (c *OpenAIClient) buildOpenAIRequest(ctx context.Context, messages []core.FluxMessage, opts core.ChatOptions, stream bool) (*http.Request, []byte, error) {
	messages = core.SanitizeMessages(messages)
	if opts.Model == "" {
		return nil, nil, fmt.Errorf("flux: model is required for %s", c.providerName)
	}

	reqBody := c.buildRequest(messages, opts, stream)
	body, err := json.Marshal(reqBody)
	if err != nil {
		return nil, nil, fmt.Errorf("flux: marshal openai request: %w", err)
	}
	if len(body) > maxOpenAIRequestSize {
		return nil, nil, fmt.Errorf("flux: request size %d bytes exceeds OpenAI limit of %d bytes", len(body), maxOpenAIRequestSize)
	}
...
func (c *OpenAIClient) Chat(ctx context.Context, messages []core.FluxMessage, opts core.ChatOptions) (*core.FluxResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("flux: %s request failed: %w", c.providerName, err)
	}
	defer func() {
	...
	if err := json.NewDecoder(resp.Body).Decode(&or); err != nil {
		return nil, fmt.Errorf("flux: failed to decode %s response: %w", c.providerName, err)
	}

	result := &core.FluxResponse{FinishReason: "unknown", RequestID: requestID}
	if len(or.Choices) > 0 {
		ch := or.Choices[0]
...
func (c *OpenAIClient) Ping(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("flux: %s ping failed: %w", c.providerName, err)
	}
	c.setHeaders(req)
	...
	if resp.StatusCode == 401 {
		return fmt.Errorf("flux: %s: invalid API key", c.providerName)
	}
	return nil
}
