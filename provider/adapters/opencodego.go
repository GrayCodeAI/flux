...
import (
	...
	"github.com/GrayCodeAI/flux/catalog/opencodego"
)
...
// NewOpenCodeGoClient builds an OpenCode Go provider provider.
...
func (c *OpenCodeGoClient) Chat(ctx context.Context, messages []core.FluxMessage, opts core.ChatOptions) (*core.FluxResponse, error) {
...
func (c *OpenCodeGoClient) StreamChat(ctx context.Context, messages []core.FluxMessage, opts core.ChatOptions) (*core.StreamResult, error) {
	opts.Model = opencodego.NativeModelID(opts.Model)
	if opencodego.UsesMessagesAPI(opts.Model) {
...
func openCodeGoMessagesFallback(primaryErr error, primaryResp *core.FluxResponse) bool {
	if primaryErr != nil {
