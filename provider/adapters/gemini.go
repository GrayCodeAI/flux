...
const geminiSharedParserEnvVar = "FLUX_GEMINI_SHARED_PARSER"
...
func (c *GeminiClient) Chat(ctx context.Context, messages []core.FluxMessage, opts core.ChatOptions) (*core.FluxResponse, error) {
	if opts.Model == "" {
	...
	if err != nil {
		return nil, fmt.Errorf("flux: gemini request creation failed: %w", err)
	}
	c.setHeaders(req)
	...
	if err != nil {
		return nil, fmt.Errorf("flux: gemini request failed: %w", err)
	}
	defer func() {
	...
	if err != nil {
		return nil, fmt.Errorf("flux: gemini response read failed: %w", err)
	}

	fluxResp, err := c.parseResponse(respBody, requestID)
	if err != nil {
		return nil, err
	}
...
func (c *GeminiClient) StreamChat(ctx context.Context, messages []core.FluxMessage, opts core.ChatOptions) (*core.StreamResult, error) {
	if opts.Model == "" {
		return nil, fmt.Errorf("flux: model is required for gemini")
	}
...
func (c *GeminiClient) Ping(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("flux: gemini ping request creation failed: %w", err)
	}
	c.setHeaders(req)
	resp, err := c.httpClient.Do(req)
	if err != nil {
		return fmt.Errorf("flux: gemini ping failed: %w", err)
	}
	defer func() {
		if err := resp.Body.Close(); err != nil {
			slog.Warn("gemini: close response body", "error", err)
	...
	if resp.StatusCode == 401 {
		return fmt.Errorf("flux: gemini: invalid API key")
	}
	return nil
}
...
func (c *GeminiClient) buildBody(messages []core.FluxMessage, opts core.ChatOptions) ([]byte, error) {
	...
	if len(data) > maxGeminiRequestSize {
		return nil, fmt.Errorf("flux: request size %d bytes exceeds Gemini limit of %d bytes", len(data), maxGeminiRequestSize)
	}
	return data, nil
}
...
func (c *GeminiClient) parseResponse(data []byte, requestID string) (*core.FluxResponse, error) {
	var gr geminiResponse
	if err := json.Unmarshal(data, &gr); err != nil {
		return nil, fmt.Errorf("flux: gemini response parse failed: %w", err)
	}
	if len(gr.Candidates) == 0 {
		...
		if gr.PromptFeedback != nil && gr.PromptFeedback.BlockReason != "" {
			...
			return nil, fmt.Errorf("flux: gemini blocked prompt: %s", msg)
		}
		return nil, fmt.Errorf("flux: gemini returned no candidates")
	}
...
// core.ParseSSEStream) into FluxStreamEvents. Handles text, tool calls
	...
	go func() {
		...
		for {
			...
			case evt, ok := <-sseEvents:
				...
				for _, part := range candidate.Content.Parts {
					if part.Text != "" {
						core.Emit(ctx, ch, core.FluxStreamEvent{Type: "content", Content: part.Text})
					}
					if part.FunctionCall != nil {
						tc := &core.ToolCall{
							Name:      part.FunctionCall.Name,
...
func (c *GeminiClient) streamLoop(ctx context.Context, body io.ReadCloser, events chan<- core.FluxStreamEvent) {
	defer close(events)
...
func (c *GeminiClient) processStreamChunkWithUsage(ctx context.Context, data string, events chan<- core.FluxStreamEvent, pendingUsage **core.FluxUsage) bool {
	...
	for _, part := range candidate.Content.Parts {
		if part.Text != "" {
			select {
			case events <- core.FluxStreamEvent{Type: "content", Content: part.Text}:
			case <-ctx.Done():
				return false
			}
		}
...
// HTTPClient returns the configured transport provider.
...
func ProcessGeminiStream(ctx context.Context, events <-chan core.SSEEvent, logger *slog.Logger) <-chan core.FluxStreamEvent {
