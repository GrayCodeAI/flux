...
func (c *VertexClient) Chat(ctx context.Context, messages []core.FluxMessage, opts core.ChatOptions) (*core.FluxResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("flux: vertex request creation failed: %w", err)
	}
	c.setHeaders(req)
	...
	if err != nil {
		return nil, fmt.Errorf("flux: vertex request failed: %w", err)
	}
	defer func() {
	...
	if err := json.NewDecoder(resp.Body).Decode(&ar); err != nil {
		return nil, fmt.Errorf("flux: vertex decode failed: %w", err)
	}

	fluxResp := ParseAnthropicResponse(ar, requestID, "")
...
func (c *VertexClient) StreamChat(ctx context.Context, messages []core.FluxMessage, opts core.ChatOptions) (*core.StreamResult, error) {
	messages = core.SanitizeMessages(messages)
	if opts.Model == "" {
		return nil, fmt.Errorf("flux: model is required for vertex")
	}
...
func (c *VertexClient) Ping(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("flux: vertex ping failed: %w", err)
	}
	_ = resp.Body.Close()
	if resp.StatusCode == 401 || resp.StatusCode == 403 {
		return fmt.Errorf("flux: vertex: invalid credentials")
	}
	return nil
}
...
func (c *VertexClient) buildBody(messages []core.FluxMessage, opts core.ChatOptions, stream bool) ([]byte, error) {
