...
// Audio: POST /v1/audio/transcriptions). They give rho's pluggable
...
// NewImageClient creates an image provider. baseURL defaults to
...
func NewImageClient(apiKey, baseURL string) *ImageClient {
	...
	return &ImageClient{apiKey: apiKey, baseURL: baseURL, httpClient: core.NewPooledHTTPClient(2 * time.Minute)}
}
...
func (c *ImageClient) Generate(ctx context.Context, prompt, model, size string, n int) ([][]byte, []string, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("flux: image generate: %w", err)
	}
	defer func() { _ = resp.Body.Close() }()
	if resp.StatusCode != http.StatusOK {
		errBody, _ := io.ReadAll(io.LimitReader(resp.Body, 4096))
		return nil, nil, fmt.Errorf("flux: image API %d: %s", resp.StatusCode, strings.TrimSpace(string(errBody)))
	}
	var out ImageGenResponse
	if err := json.NewDecoder(resp.Body).Decode(&out); err != nil {
		return nil, nil, fmt.Errorf("flux: image decode: %w", err)
	}
	...
	for _, d := range out.Data {
		if d.B64JSON != "" {
			...
			if derr != nil {
				return nil, nil, fmt.Errorf("flux: image b64 decode: %w", derr)
			}
			imgs = append(imgs, b)
		}
		if d.URL != "" {
...
// NewAudioClient creates a transcription provider.
func NewAudioClient(apiKey, baseURL string) *AudioClient {
	if baseURL == "" {
		baseURL = "https://api.openai.com"
	}
	return &AudioClient{apiKey: apiKey, baseURL: baseURL, httpClient: core.NewPooledHTTPClient(2 * time.Minute)}
}
...
func (c *AudioClient) Transcribe(ctx context.Context, r TranscriptionRequest) (string, error) {
	...
	resp, err := c.httpClient.Do(req)
	if err != nil {
		return "", fmt.Errorf("flux: transcribe: %w", err)
	}
	defer func() { _ = resp.Body.Close() }()
	if resp.StatusCode != http.StatusOK {
		errBody, _ := io.ReadAll(io.LimitReader(resp.Body, 4096))
		return "", fmt.Errorf("flux: audio API %d: %s", resp.StatusCode, strings.TrimSpace(string(errBody)))
	}
	var tr Transcript
	if err := json.NewDecoder(resp.Body).Decode(&tr); err != nil {
		return "", fmt.Errorf("flux: transcript decode: %w", err)
	}
	return tr.Text, nil
}
