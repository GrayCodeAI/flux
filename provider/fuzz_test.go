...
func FuzzSanitizeMessages(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, role1, content1, role2, content2, toolID string) {
		...
		if toolID != "" {
			messages = append(messages, FluxMessage{
				Role: "assistant",
				ToolUse: []ToolCall{
...
// security-relevant parser in flux, since it scans untrusted LLM output for
