...
// staticProviderNames is a snapshot of every built-in provider, captured at
// init() time. Per-client custom providers must never alter these maps.
...
func TestProviderRegistry_NoDriftFromCatalog(t *testing.T) {
	...
	if len(staleInRuntime) > 0 {
		...
		t.Fatalf(
			"provider-registry drift: %d provider(s) in provider/adapters/provider_registry.go "+
