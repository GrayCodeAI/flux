...
func (m *MockProvider) Chat(ctx context.Context, messages []FluxMessage, opts ChatOptions) (*FluxResponse, error) {
	m.mu.Lock()
	...
	case MockModeMaxTokens:
		return &FluxResponse{Content: "partial response", FinishReason: "max_tokens", Usage: &FluxUsage{PromptTokens: 10, CompletionTokens: 100, TotalTokens: 110}}, nil
	case MockModeToolUse:
		...
		return &FluxResponse{
			...
			Usage:        &FluxUsage{PromptTokens: 10, CompletionTokens: 5, TotalTokens: 15},
		}, nil
	case MockModeFixed:
		return &FluxResponse{Content: m.Response, FinishReason: "end_turn", Usage: &FluxUsage{PromptTokens: 10, CompletionTokens: 5, TotalTokens: 15}}, nil
	default: // echo
		last := ""
		...
		return &FluxResponse{Content: "echo: " + last, FinishReason: "end_turn", Usage: &FluxUsage{PromptTokens: 10, CompletionTokens: 5, TotalTokens: 15}}, nil
	}
}

// StreamChat streams a mock response word by word.
func (m *MockProvider) StreamChat(ctx context.Context, messages []FluxMessage, opts ChatOptions) (*StreamResult, error) {
	...
	go func() {
		...
		if resp.FinishReason == "tool_use" && len(resp.ToolCalls) > 0 {
			core.Emit(streamCtx, ch, FluxStreamEvent{Type: "tool_call", ToolCall: &resp.ToolCalls[0]})
		} else {
			...
			for _, w := range words {
				...
				core.Emit(streamCtx, ch, FluxStreamEvent{Type: "content", Content: w + " "})
			}
		}
		core.Emit(streamCtx, ch, FluxStreamEvent{Type: "done"})
	}()
