...
func New(entries []RouteEntry, fallback []core.Provider, defaultRetry *RetryConfig, opts ...Option) *Router {
	total := 0
...
func (r *Router) Chat(ctx context.Context, messages []core.FluxMessage, opts core.ChatOptions) (*core.FluxResponse, error) {
...
func (r *Router) StreamChat(ctx context.Context, messages []core.FluxMessage, opts core.ChatOptions) (*core.StreamResult, error) {
	provider, retry := r.selectProvider()
	r.stratState.beginInFlight(provider.Name())
	start := time.Now()
...
func (r *Router) selectProvider() (core.Provider, RetryConfig) {
	e := r.selectEntry()
	rc := r.defaultRetry
	if e.Retry != nil {
		rc = *e.Retry
...
func (r *Router) chatWithRetry(ctx context.Context, p core.Provider, messages []core.FluxMessage, opts core.ChatOptions, cfg RetryConfig) (*core.FluxResponse, error) {
	var lastErr error
...
func (r *Router) streamWithRetry(ctx context.Context, p core.Provider, messages []core.FluxMessage, opts core.ChatOptions, cfg RetryConfig) (*core.StreamResult, error) {
	var lastErr error
	for attempt := 0; attempt <= cfg.MaxRetries; attempt++ {
...
func (r *Router) recordUsage(name string, resp *core.FluxResponse) {
