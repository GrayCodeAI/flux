...
var testCatalog *catalog.CompiledCatalog

// Tests below that read or write the package-level testCatalog run
...
func TestFeatureDefaultProviders_NoCatalog(t *testing.T) {
	orig := testCatalog
	defer func() { testCatalog = orig }()
	testCatalog = nil

	pf := NewProviderFeatures(testCatalog)
...
func TestFeatureSetFromCatalog_OverridesHardcoded(t *testing.T) {
	// Save and restore the global testCatalog
	orig := testCatalog
	defer func() { testCatalog = orig }()

	// Inject a mock catalog with per-model capabilities
	testCatalog = &catalog.CompiledCatalog{
