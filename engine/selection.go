...
// NormalizeProviderID resolves provider aliases to Flux's runtime identifier.
...
func (e *Engine) SetActiveProvider(ctx context.Context, providerID string) error {
	...
	if providerID == "" {
		return invalid("set_active_provider", "flux engine: provider id is required")
	}
...
func (e *Engine) SetSelection(ctx context.Context, providerID, modelID string) error {
	...
	if modelID == "" {
		return invalid("set_selection", "flux engine: model id is required")
	}
	unlock := lockProviderStatePath(e.providerConfigPath)
	defer unlock()
	cfg, err := e.loadProviderConfigStrict()
...
func selectionModelUnavailable(providerID, modelID string) error {
	message := fmt.Sprintf("flux engine: model %q is not available", modelID)
	if providerID != "" {
		message = fmt.Sprintf("flux engine: model %q is not available through %q", modelID, providerID)
	}
