...
func (e *Engine) RefreshCatalog(ctx context.Context, providerID string) (CatalogSnapshot, error) {
	...
	if providerID = strings.TrimSpace(providerID); providerID != "" {
		...
		if !ok {
			return CatalogSnapshot{}, &Error{Code: ErrorInvalidRequest, Operation: "refresh_catalog", Provider: providerID, Message: "flux engine: unknown provider"}
		}
	...
	if result == nil || result.Compiled == nil {
		return CatalogSnapshot{}, &Error{Code: ErrorCatalogUnavailable, Operation: "refresh_catalog", Provider: providerID, Message: "flux engine: catalog refresh returned no compiled catalog"}
	}
...
func (e *Engine) ListLiveModels(ctx context.Context, providerID string) ([]Model, error) {
	...
	if providerID == "" {
		return nil, invalid("list_live_models", "flux engine: provider id is required")
	}
	if _, custom := e.customGateway(providerID); custom {
		return nil, invalid("list_live_models", "flux engine: custom gateway live model listing is not supported")
	}
	compiled, _ := catalog.LoadCatalog(ctx, catalog.LoadCatalogOptions{CachePath: e.catalogPath})
...
func listPublicModels(ctx context.Context, providerID, catalogURL string) ([]Model, error) {
	...
	default:
		return nil, invalid("list_public_models", "flux engine: gateway has no public model metadata source")
	}
