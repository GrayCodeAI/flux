...
func (e *Engine) SaveCredential(ctx context.Context, providerID, secret string) (CredentialStatus, error) {
	...
	if providerID == "" {
		return CredentialStatus{}, invalid("save_credential", "flux engine: provider id is required")
	}
	...
	if envKey == "" {
		return CredentialStatus{}, invalid("save_credential", "flux engine: provider has no credential target")
	}
	if err := e.secretStore.Set(ctx, credentials.AccountForEnv(envKey), prepared); err != nil {
		return CredentialStatus{}, &Error{Code: ErrorInternal, Operation: "save_credential", Provider: providerID, Message: "flux engine: could not save credential", Cause: err}
	}
...
func (e *Engine) RemoveCredential(ctx context.Context, providerID string) error {
	...
	if gateway, ok := e.customGateway(providerID); ok {
		...
		if err := e.secretStore.Delete(ctx, credentials.AccountForEnv(gateway.CredentialEnv)); err != nil && !errors.Is(err, credentials.ErrNotFound) {
			return &Error{Code: ErrorInternal, Operation: "remove_credential", Provider: gateway.ID, Message: "flux engine: could not remove credential", Cause: err}
		}
		return nil
	}
	...
	for _, envKey := range e.CredentialEnvKeys(providerID) {
		if err := e.secretStore.Delete(ctx, credentials.AccountForEnv(envKey)); err != nil && !errors.Is(err, credentials.ErrNotFound) {
			return &Error{Code: ErrorInternal, Operation: "remove_credential", Provider: providerID, Message: "flux engine: could not remove credential", Cause: err}
		}
	}
	return nil
}
...
func (e *Engine) CredentialStatus(ctx context.Context, providerID string) (CredentialStatus, error) {
	...
	if gateway, ok := e.customGateway(providerID); ok {
		...
		if err != nil {
			return CredentialStatus{}, &Error{Code: ErrorInternal, Operation: "credential_status", Provider: gateway.ID, Message: "flux engine: could not read credential status", Cause: err}
		}
	...
	for _, envKey := range e.CredentialEnvKeys(providerID) {
		...
		if err != nil {
			return CredentialStatus{}, &Error{Code: ErrorInternal, Operation: "credential_status", Provider: providerID, Message: "flux engine: could not read credential status", Cause: err}
		}
...
func (e *Engine) saveCustomGatewayCredential(ctx context.Context, gateway CustomGateway, secret string) (CredentialStatus, error) {
	...
	if err := e.secretStore.Set(ctx, credentials.AccountForEnv(gateway.CredentialEnv), secret); err != nil {
		return CredentialStatus{}, &Error{Code: ErrorInternal, Operation: "save_credential", Provider: gateway.ID, Message: "flux engine: could not save credential", Cause: err}
	}
